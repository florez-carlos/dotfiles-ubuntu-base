...
build:
	...
	-t $$REGISTRY/$(GIT_USER_USERNAME)/dev-env-ubuntu-base-img:v$$IMAGE_VERSION .

build-latest:
	@docker build --no-cache \
		-t $$REGISTRY/$(GIT_USER_USERNAME)/dev-env-ubuntu-base-img:latest . --progress plain
...
run:
	docker run -it --rm --name dev-env-ubuntu-base-cont -d $$REGISTRY/$(GIT_USER_USERNAME)/dev-env-ubuntu-base-img:v$$IMAGE_VERSION
